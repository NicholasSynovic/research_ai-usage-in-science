...
class OpenAlexRunner(Runner):
    def __init__(self, logger: Logger, email: str, db: DB) -> None:
        ...
        self.db: DB = db
        self.search_template: Template = Template(template="https://api.openalex.org/works?per-page=100&mailto=${email}&filter=doi:${dois}")
        ...
        dois: list[str] = ["https://doi.org/" + doi for doi in pandas.read_sql_query(sql="SELECT DISTINCT doi FROM articles;", con=self.db.engine)["doi"].tolist()]
        self.doi_chunks: list[list[str]] = [dois[i:i + 50] for i in range(0, len(dois), 50)]
    ...
    def search(self)    -> list[MetadataModel]:
        ...
        with Bar("Searching OpenAlex for DOI metadata...", max=len(self.doi_chunks)) as bar:
            chunk: list[str]
            for chunk in self.doi_chunks:
                url: str = self.search_template.substitute(email=self.email, dois="|".join(chunk),)
                ...
                for result in resp.json()["results"]:
                    topic_0, topic_1, topic_2 = self.extract_topics(topics=result["topics"])

                    data.append(MetadataModel(
                        timestamp=timestamp, doi=result["doi"].replace("https://doi.org/", ""),
    ...
    def execute(self)    ->  int:
        ...
        # later
        search_table_row_count: int = self.db.get_last_row_id(
            table_name="openalex"
        )
        ...
        self.logger.info(
            msg=f"Searched {len(searches)} documents"
        )
