...
def get_aa_data(db: DB) -> DataFrame:
    ...
    return df[["uses_dl", "plos_paper_id", "topic_0", "topic_1", "topic_2"]]
...
def compute(gpt_df: DataFrame) -> DataFrame:
    ...
    topics.value_counts().to_csv(path_or_buf="topics_uses_dl.csv")
...
def main(db_path: Path, gpt_path: Path) -> None:
    ...
    compute(gpt_df=gpt_df[gpt_df["uses_dl"] == True])
    compute(gpt_df=aa_df[aa_df["uses_dl"] == True])
