...
def bmj_parse_response(df: pd.DataFrame) -> list[ArticleModel]:
        ...
        with Bar(
            "Extracting articles from search results...", max=df.shape[0]
        ) as bar:
            row: pd.Series
...
def parse_searches_for_articles(searches: pd.DataFrame) -> list[ArticleModel]:
        ...
        for key in keys:
            match key:
                case "bmj": data.extend(bmj_parse_response(searches[searches["megajournal"] == "BMJ"]))
                case "f1000": data.extend(f1000_parse_response(searches[searches["megajournal"] == "F1000"]))
                case "frontiersin": data.extend(frontiersin_parse_response(searches[searches["megajournal"] == "FrontiersIn"]))
                case "plos": data.extend(plos_parse_response(searches[searches["megajournal"] == "PLOS"]))
...
def main() -> None:
    ...
    article_df: pd.DataFrame = pd.concat(objs=[a.to_df for a in articlemodels], ignore_index=True)

    article_df.to_sql(name="articles", con=engine, if_exists="append", index=True, index_label="_id")
